...
def run_static_backtest():
    """
    Runs a portfolio-level pairs trading backtest from pre-processed CSV files
    with proper capital, leverage, and beta-neutral position sizing.
    """
    # --- 1. CONFIGURATION ---
    DATA_DIR = 'static_backtest_data'
    ...
    for current_date in tqdm(master_timeline, desc="Running Static Backtest"):
        ...
        for pair_name, pair_df in all_pairs_data.items():
            ...
            hedge_ratio = pair_df.loc[current_date, 'hedge_ratio'] # Get the static hedge ratio
            ...
            if position:
                ...
                if not exit_signal and days_held > MAX_HOLD_DAYS:
    ...
    output_filename = 'detailed_trades_static.csv'
